...
def make_wrapped_procgen_env(gym_id, num_levels=0, starting_level=0, distribution_mode="easy"):
    ...
    wrapped_env = gym.wrappers.NormalizeReward(wrapped_env)
    print(f"normalize reward:\t {wrapped_env.update_running_mean}")
...
class CustomProcgenEnvWrapper(gym.Env):
    def __init__(self, env, num_envs=1):
        if num_envs > 1:
            raise AttributeError("More than 1 not supported")
        self.env = env
    ...
    def reset(self, *args, **kwargs):
        ...
        obs, i = self.env.reset(*args, **kwargs)
        return obs, i
