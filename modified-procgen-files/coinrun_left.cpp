...
const std::string NAME = "coinrun_left";
...
const std::vector<std::string> WALKING_ENEMIES = {"slimeBlock", "slimePurple", "slimeBlue", "slimeGreen", "mouse", "snail", "ladybug", "wormGreen", "wormPink"};
const std::vector<std::string> PLAYER_THEME_COLORS = {"Beige", "Blue", "Green", "Pink", "Yellow"};
const std::vector<std::string> GROUND_THEMES = {"Dirt", "Grass", "Planet", "Sand", "Snow", "Stone"};


const int NUM_GROUND_THEMES = (int)(GROUND_THEMES.size());

class CoinRunLeft : public BasicAbstractGame {
  public:
    ...
    std::pair<int, int> generate_coin_to_the_left() {
        ...
        for (int section_idx = 0; section_idx < num_sections; section_idx++) {
            ...
            if (use_pit) {
                ...
                // Fill ground from right to left
                fill_ground_block(curr_x - x1, 0, x1, curr_y);
                fill_ground_block(curr_x - dx, 0, x2, curr_y);
                ...
                if (danger_type == 0) {
                    ...
                    fill_lava_block(curr_x - x1 - pit_width, 1, pit_width, lava_height);
                } else if (danger_type == 1) {
                    for (int ei = 1; ei < pit_width + 1; ei++) {
                ...
                if (pit_width > 4) {
                    ...
                    fill_ground_block(curr_x - x1 - x3 - w1, curr_y - 1, w1, 1);
                    // fill_ground_block(curr_x + x1 + x3, curr_y - 1, w1, 1);
                }
            } else {
                fill_ground_block(curr_x - dx, 0, dx, curr_y);
        ...
        set_obj(curr_x - 1, curr_y, GOAL); // Place goal at the end (left side now)

        fill_ground_block(curr_x - 1, 0, 1, curr_y);
        // fill_ground_block(curr_x, 0, 1, curr_y);
        // fill_elem(curr_x + 1, 0, main_width - curr_x - 1, main_height, WALL_MID);
        fill_elem(0, 0, curr_x - 1, main_height, WALL_MID); // Fill left side with walls

        return std::make_pair(main_width - 2, 1);
    }
    ...
    void game_reset() override {
        ...
        std::pair<int, int> myPair = generate_coin_to_the_left();
        agent->x = myPair.first + agent->rx;
        agent->y = myPair.second + agent->ry;
    }
...
REGISTER_GAME(NAME, CoinRunLeft);
