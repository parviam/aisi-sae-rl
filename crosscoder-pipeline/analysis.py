...
fig.write_image("scatter_plot.png")  # Save as PNG
...
cosine_sims = (cross_coder.W_dec[:, 0, :] *
               cross_coder.W_dec[:, 1, :]).sum(dim=-1) / (
               cross_coder.W_dec[:, 0, :].norm(dim=-1) *
               cross_coder.W_dec[:, 1, :].norm(dim=-1)
)
cosine_sims.shape
fig = px.histogram(
...
fig.write_image("scatter_plot2.png")  # Save as PNG
