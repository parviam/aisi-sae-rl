...
class Buffer:
    """
    This defines a data buffer,
    to store a stack of acts across both model,
    that can be used to train the autoencoder.
    It'll automatically run the model,
    to generate more when it gets halfway empty.
    """
    ...
    def refresh(self):
        ...
        with torch.autocast("cuda", torch.bfloat16):
            ...
            for _ in tqdm.trange(0, num_batches, self.cfg["model_batch_size"]):
                ...
                acts = torch.stack(
                    [cache_A[self.cfg["hook_point"]],
                     cache_B[self.cfg["hook_point"]]],
                    dim=0,
                )
                ...
                acts = einops.rearrange(
                    acts,
                    "n_layers batch seq_len d_model -> " /
                    "(batch seq_len) n_layers d_model",
                )
